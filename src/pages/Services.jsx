              ...
              <GlassCard key={service.name} delay={i * 0.03}>
                <div className="group">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <Link to={`/book?service=${encodeURIComponent(service.name)}`} className="rounded-lg bg-blue-500/15 px-3 py-2 text-xs font-semibold text-blue-300 transition hover:bg-blue-500/25">
                      Book Now
                    </Link>
                    {!service.isCms && (
                      <Link to={`/services/${service.slug}`} className="text-xs text-blue-400 font-medium flex items-center gap-1 transition hover:text-blue-300">
